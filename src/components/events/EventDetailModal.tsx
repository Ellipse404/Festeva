...
import { CashfreeCheckoutModal } from '../payments/CashfreeCheckoutModal';
...
export const EventDetailModal: React.FC = () => {
  ...
  // Cashfree Checkout Modal State
  const [checkoutOrder, setCheckoutOrder] = useState<ICreateOrderResponse | null>(null);
  ...
  const handleBuy = async () => {
    ...
    if (quantity > selectedEvent.availableSeats) {
      toast.error(
        MESSAGES.TOAST.SEATS_REMAINING_ERROR(selectedEvent.availableSeats),
      );
      return;
    }
    ...
    try {
      // 1. Create Cashfree Order & reserve seats on Nest backend
      ...
      // 2. Open Cashfree Checkout Modal for Payment
      setCheckoutOrder(orderRes);
      setIsCheckoutOpen(true);
    } catch (err: any) {
      toast.error(err?.message || 'Failed to initiate payment. Please retry.');
    } finally {
  ...
  const handlePaymentSuccess = () => {
    if (selectedEvent) {
      buyTicket(selectedEvent.id, quantity, selectedEvent);
    }
              ...
              >
                ...
                <Stack direction="row" spacing={2} alignItems="center">
                  ...
                  <Button
                    ...
                    fullWidth
                    size="large"
                    startIcon={isProcessing ? <CircularProgress size={20} color="inherit" /> : <TicketIcon />}
                    onClick={handleBuy}
                    disabled={selectedEvent.availableSeats === 0 || isProcessing}
